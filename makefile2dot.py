...
def makefile2dot():
    ...
    for line in _single_dot_dep_emitter(
        _dependency_emitter(_line_emitter("".join(stdin).split("\n")))
    ):
        stdout.write(line)
    stdout.write("}\n")
...
class Test(unittest.TestCase):
    def test_emitters(self):
        ...
        self.assertEqual(
            list(
                _dependency_emitter(
                    _line_emitter(["macro", "out:dep1\\", " dep2", "\tcommand"])
                )
            ),
            [("out", "dep1 dep2")],
        )
        ...
        self.assertEqual(
            list(_dependency_emitter(_line_emitter(["default:", "", ""]))),
            [("default", "")],
        )
        ...
        self.assertEqual(
            list(_single_dot_dep_emitter(_dependency_emitter(_line_emitter(inp)))),
            [
                '\t"out"\n',
                '\t"dep1" -> "out"\n',
                '\t"dep2" -> "out"\n',
